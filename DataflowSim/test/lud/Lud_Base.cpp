...
void LudTest::lud_Base(Debug* debug)
{
    ...
    Lse* lse_ld_akj = new Lse(LSE_QUEUE_SIZE, 0, false, memSys, Base_inner_loop_speedup);  // Load a[k][j]
    Lse* lse_st_aij = new Lse(LSE_QUEUE_SIZE, 0, true, memSys, Base_outer_loop_speedup);  // Store a[i][j]

    Lse* lse_ld_aji = new Lse(LSE_QUEUE_SIZE, 0, false, memSys, Base_outer_loop_speedup);  // load a[j][i]
    Lse* lse_ld_ajk = new Lse(LSE_QUEUE_SIZE, 0, false, memSys, Base_inner_loop_speedup);  // load a[j][k]
    Lse* lse_ld_aki = new Lse(LSE_QUEUE_SIZE, 0, false, memSys, Base_inner_loop_speedup);  // Load a[k][i]
    Lse* lse_st_aji = new Lse(LSE_QUEUE_SIZE, 0, true, memSys, Base_outer_loop_speedup);  // Store a[j][i]
    ...
    ChanBase* chan_aij_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, ADD + MUL, Base_outer_loop_speedup);

    ChanBase* chan_sum_loop_j1 = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, 0, Base_outer_loop_speedup);
    chan_sum_loop_j1->keepMode = 1;

    ChanBase* chan_sum_loop_j1_drainMode = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, 0, Base_outer_loop_speedup);
    chan_sum_loop_j1_drainMode->drainMode = 1;

    // loop j2
    ChanBase* chan_i_relay_loop_j2 = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, 0, Base_outer_loop_speedup);  // Relay channel in loop j2 for chan_i_lc
    chan_i_relay_loop_j2->keepMode = 1;

    ChanBase* chan_aji_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, ADD + MUL, Base_outer_loop_speedup);

    ChanBase* chan_sum_loop_j2 = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, 0, Base_outer_loop_speedup);
    chan_sum_loop_j2->keepMode = 1;

    ChanBase* chan_aii_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, ADD + MUL, Base_outer_loop_speedup);

    ChanBase* chan_sum_loop_j2_drainMode = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, 0, Base_outer_loop_speedup);
    chan_sum_loop_j2_drainMode->drainMode = 1;

    ChanBase* chan_sum_div = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_outer_loop_speedup, DIV, Base_outer_loop_speedup);

    // loop k1
    ChanBase* chan_aik_addr = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_aik_addr_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);

    ChanBase* chan_akj_addr = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_akj_addr_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);

    ChanBase* chan_sum_loop_k1 = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_sum_loop_k1_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, MUL + SUB, Base_inner_loop_speedup);

    // loop k2
    ChanBase* chan_ajk_addr = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_ajk_addr_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);

    ChanBase* chan_aki_addr = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_aki_addr_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);

    ChanBase* chan_sum_loop_k2 = new ChanBase(BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, 0, Base_inner_loop_speedup);
    ChanBase* chan_sum_loop_k2_delay = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, MUL + SUB, Base_inner_loop_speedup);
    ...
    chan_sum_loop_j1_drainMode->addUpstream({ chan_sum_loop_k1_delay });
    chan_sum_loop_j1_drainMode->addDownstream({ lse_st_aij });

    lse_st_aij->addUpstream({ chan_aij_addr, chan_sum_loop_j1_drainMode });
    ...
    chan_aii_addr->addUpstream({ lc_j2->loopVar, chan_i_lc });
    ...
    chan_sum_loop_j2_drainMode->addUpstream({ chan_sum_loop_k2_delay });
    chan_sum_loop_j2_drainMode->addDownstream({ chan_sum_div });

    chan_sum_div->addUpstream({ chan_sum_loop_j2_drainMode, lse_ld_aii });
    ...
    chan_akj_addr->addDownstream({ chan_akj_addr_delay });

    chan_akj_addr_delay->addUpstream({ chan_akj_addr });
    chan_akj_addr_delay->addDownstream({ lse_ld_akj });

    lse_ld_akj->addUpstream({ chan_akj_addr_delay });
    lse_ld_akj->addDownstream({ chan_sum_loop_k1 });

    chan_aik_addr->addUpstream({ lc_k1->loopVar, chan_i_relay_loop_j1 });
    chan_aik_addr->addDownstream({ chan_aik_addr_delay });

    chan_aik_addr_delay->addUpstream({ chan_aik_addr });
    chan_aik_addr_delay->addDownstream({ lse_ld_aik });

    lse_ld_aik->addUpstream({ chan_aik_addr_delay });
    lse_ld_aik->addDownstream({ chan_sum_loop_k1 });

    chan_sum_loop_k1->addUpstream({ lse_ld_aik, lse_ld_akj, chan_sum_loop_j1 });
    chan_sum_loop_k1->addDownstream({ chan_sum_loop_k1_delay });

    chan_sum_loop_k1_delay->addUpstream({ chan_sum_loop_k1 });
    chan_sum_loop_k1_delay->addDownstream({ chan_sum_loop_j1_drainMode });

    // loop k2
    chan_ajk_addr->addUpstream({ lc_k2->loopVar, chan_j2_lc });
    chan_ajk_addr->addDownstream({ chan_ajk_addr_delay });

    chan_ajk_addr_delay->addUpstream({ chan_ajk_addr });
    chan_ajk_addr_delay->addDownstream({ lse_ld_ajk });

    lse_ld_ajk->addUpstream({ chan_ajk_addr_delay });
    lse_ld_ajk->addDownstream({ chan_sum_loop_k2 });

    chan_aki_addr->addUpstream({ lc_k2->loopVar, chan_i_relay_loop_j2 });
    chan_aki_addr->addDownstream({ chan_aki_addr_delay });

    chan_aki_addr_delay->addUpstream({ chan_aki_addr });
    chan_aki_addr_delay->addDownstream({ lse_ld_aki });

    lse_ld_aki->addUpstream({ chan_aki_addr_delay });
    lse_ld_aki->addDownstream({ chan_sum_loop_k2 });

    chan_sum_loop_k2->addUpstream({ lse_ld_ajk, lse_ld_aki, chan_sum_loop_j2 });
    chan_sum_loop_k2->addDownstream({ chan_sum_loop_k2_delay });

    chan_sum_loop_k2_delay->addUpstream({ chan_sum_loop_k2 });
    chan_sum_loop_k2_delay->addDownstream({ chan_sum_loop_j2_drainMode });


    //*** LC addPort : getAct, sendAct, getEnd, sendEnd
    lc_i->addPort({ }, { chan_i_lc }, { lc_j1->sendEnd }, { end });
    ...
    lc_j2->addPort({ chan_i_lc }, { chan_j2_lc, chan_i_relay_loop_j2, chan_aji_addr, chan_aii_addr }, { lse_st_aji }, { /*lc_i->getEnd*/ });
    ...
    while (iter < max_iter)
    {
        ...
        chan_akj_addr_delay->get();
        chan_akj_addr_delay->value = chan_akj_addr_delay->assign(chan_akj_addr);

        chan_aik_addr->get();
        chan_aik_addr->value = chan_aik_addr->assign(chan_i_relay_loop_j1) * matrix_size + chan_aik_addr->assign(lc_k1->loopVar);

        chan_aik_addr_delay->get();
        chan_aik_addr_delay->value = chan_aik_addr_delay->assign(chan_aik_addr);
        ...
        chan_sum_loop_k1_delay->get();
        chan_sum_loop_k1_delay->value = chan_sum_loop_k1_delay->assign(chan_sum_loop_k1);

        //** loop Lc_j1 tail
        chan_sum_loop_j1_drainMode->get();
        chan_sum_loop_j1_drainMode->value = chan_sum_loop_j1_drainMode->assign(chan_sum_loop_k1_delay);

        lse_st_aij->get();
        lse_st_aij->value = lse_st_aij->assign();
        if (!lse_st_aij->chanBuffer[0].empty() && !lse_st_aij->chanBuffer[1].empty())
        {
            int addr_aij = lse_st_aij->chanBuffer[0].front().value;
            int data_aij = lse_st_aij->chanBuffer[1].front().value;
            matrix[addr_aij] = data_aij;
        }
        ...
        chan_ajk_addr_delay->get();
        chan_ajk_addr_delay->value = chan_ajk_addr_delay->assign(chan_ajk_addr);

        chan_aki_addr->get();
        chan_aki_addr->value = chan_aki_addr->assign(lc_k2->loopVar) * matrix_size + chan_aki_addr->assign(chan_i_relay_loop_j2);

        chan_aki_addr_delay->get();
        chan_aki_addr_delay->value = chan_aki_addr_delay->assign(chan_aki_addr);
        ...
        chan_sum_loop_k2_delay->get();
        chan_sum_loop_k2_delay->value = chan_sum_loop_k2_delay->assign(chan_sum_loop_k2);

        //** loop Lc_j2 tail
        chan_sum_loop_j2_drainMode->get();
        chan_sum_loop_j2_drainMode->value = chan_sum_loop_j2_drainMode->assign(chan_sum_loop_k2_delay);

        chan_sum_div->get();
        if (chan_sum_div->assign(lse_ld_aii) != 0)
        {
            chan_sum_div->value = chan_sum_div->assign(chan_sum_loop_j2_drainMode) / chan_sum_div->assign(lse_ld_aii);
        }

        lse_st_aji->get();
        lse_st_aji->value = lse_st_aji->assign();
        ...
        if (iter > 0)
        {
            ...
            debug->chanPrint("chan_aij_addr", chan_aij_addr);
            debug->lsePrint("lse_ld_aij", lse_ld_aij);
            debug->chanPrint("chan_sum_loop_j1", chan_sum_loop_j1);
            debug->chanPrint("chan_sum_loop_j1_drainMode", chan_sum_loop_j1_drainMode);
            debug->lsePrint("lse_st_aij", lse_st_aij);
            // loop k1
            debug->getFile() << std::endl;
            debug->getFile() << "*****************  k1  *****************" << std::endl;
            debug->chanPrint("lc_k1->loopVar", lc_k1->loopVar);
            debug->chanPrint("chan_aik_addr", chan_aik_addr);
            debug->lsePrint("lse_ld_aik", lse_ld_aik);
            debug->chanPrint("chan_akj_addr", chan_akj_addr);
            debug->lsePrint("lse_ld_akj", lse_ld_akj);
            debug->chanPrint("chan_sum_loop_k1", chan_sum_loop_k1);
            // loop j2
            debug->getFile() << std::endl;
            ...
            debug->chanPrint("chan_aji_addr", chan_aji_addr);
            debug->lsePrint("lse_ld_aji", lse_ld_aji);
            debug->chanPrint("chan_sum_loop_j2", chan_sum_loop_j2);
            debug->chanPrint("chan_aii_addr", chan_aii_addr);
            debug->lsePrint("lse_ld_aii", lse_ld_aii);
            debug->chanPrint("chan_sum_loop_j2_drainMode", chan_sum_loop_j2_drainMode);
            debug->chanPrint("chan_sum_div", chan_sum_div);
            debug->lsePrint("lse_st_aji", lse_st_aji);
            // loop k2
            debug->getFile() << std::endl;
            debug->getFile() << "*****************  k2  *****************" << std::endl;
            debug->chanPrint("lc_k2->loopVar", lc_k2->loopVar);
            debug->chanPrint("chan_aki_addr", chan_aki_addr);
            debug->lsePrint("lse_ld_aki", lse_ld_aki);
            debug->chanPrint("chan_ajk_addr", chan_ajk_addr);
            debug->lsePrint("lse_ld_ajk", lse_ld_ajk);
            debug->chanPrint("chan_sum_loop_k2", chan_sum_loop_k2);
...
//
//    // loop k1
//    ChanBase* chan_aik_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);
//    ChanBase* chan_akj_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);
//    ChanBase* chan_sum_loop_k1 = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, MUL + SUB, Base_inner_loop_speedup);
//
//    // loop k2
//    ChanBase* chan_ajk_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);
//    ChanBase* chan_aki_addr = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, ADD + MUL, Base_inner_loop_speedup);
//    ChanBase* chan_sum_loop_k2 = new ChanBase(2 * BASE_INPUT_BUFF_SIZE * Base_inner_loop_speedup, MUL + SUB, Base_inner_loop_speedup);
//
//
//    //*** Define interconnect
//    // Begin & end connect to the outer most loop
...
//    chan_sum_loop_j1->addDownstream({ chan_sum_loop_k1 });
//
//    chan_sum_loop_j1_drainMode->addUpstream({ chan_sum_loop_k1 });
//    chan_sum_loop_j1_drainMode->addDownstream({ lse_st_aij });
//
//    lse_st_aij->addUpstream({ chan_aij_addr, chan_sum_loop_j1_drainMode });
//    lse_st_aij->noDownstream = 1;
...
//    chan_sum_loop_j2->addDownstream({ chan_sum_loop_k2 });
//
//    chan_sum_loop_j2_drainMode->addUpstream({ chan_sum_loop_k2 });
//    chan_sum_loop_j2_drainMode->addDownstream({ chan_sum_div });
//
//    chan_sum_div->addUpstream({ chan_sum_loop_j2_drainMode, lse_ld_aii });
//    chan_sum_div->addDownstream({ lse_st_aji });
...
//
//    chan_sum_loop_k1->addUpstream({ lse_ld_aik, lse_ld_akj, chan_sum_loop_j1 });
//    chan_sum_loop_k1->addDownstream({ chan_sum_loop_j1_drainMode });
//
//    // loop k2
//    chan_ajk_addr->addUpstream({ lc_k2->loopVar, chan_j2_lc });
...
//
//    chan_sum_loop_k2->addUpstream({ lse_ld_ajk, lse_ld_aki, chan_sum_loop_j2 });
//    chan_sum_loop_k2->addDownstream({ chan_sum_loop_j2_drainMode });
//
//
//    //*** LC addPort : getAct, sendAct, getEnd, sendEnd
...
//        //** loop Lc_j1 tail
//        chan_sum_loop_j1_drainMode->get();
//        chan_sum_loop_j1_drainMode->value = chan_sum_loop_j1_drainMode->assign(chan_sum_loop_k1);
//
//        lse_st_aij->get();
//        lse_st_aij->value = lse_st_aij->assign();
//        if (!lse_st_aij->chanBuffer[0].empty() && !lse_st_aij->chanBuffer[1].empty())
...
//        //** loop Lc_j2 tail
//        chan_sum_loop_j2_drainMode->get();
//        chan_sum_loop_j2_drainMode->value = chan_sum_loop_j2_drainMode->assign(chan_sum_loop_k2);
//
//        chan_sum_div->get();
//        if (chan_sum_div->assign(lse_ld_aii) != 0)
//        {
