...
void GemmTest::gemm_base(Debug* debug)
{
    ...
    //*** Declare Lse
    Lse* lse_ld_m1 = new Lse(16, 0, false, memSys, 5);  // Load M1
    Lse* lse_ld_m2 = new Lse(16, 0, false, memSys, 5);  // Load M2
    Lse* lse_ld_partialSum = new Lse(16, 0, false, memSys, 5);  // load partial sum
    Lse* lse_st_partialSum = new Lse(16, 0, true, memSys, 5);  // Store back partial sum
    ...
    // loop kk
    ChanBase* chan_jj_relay_loop_kk = new ChanBase(16, 0, 1);  // Relay channel in loop kk for chan_jj_lc
    chan_jj_relay_loop_kk->keepMode = 1;

    // loop i
    ChanBase* chan_i_row = new ChanBase(16, 4, 1);
    chan_i_row->keepMode = 1;

    ChanBase* chan_jj_relay_loop_i = new ChanBase(16, 0, 1);  // Relay channel in loop i for chan_jj_lc
    chan_jj_relay_loop_i->keepMode = 1;

    ChanBase* chan_kk_relay_loop_i = new ChanBase(16, 0, 1);  // Relay channel in loop i for chan_kk_lc
    chan_kk_relay_loop_i->keepMode = 1;

    // loop k
    ChanBase* chan_m1_addr = new ChanBase(16, 4, 1);

    ChanBase* chan_k_row = new ChanBase(16, 4, 1);
    chan_k_row->keepMode = 1;

    ChanBase* chan_m1_getData = new ChanBase(16, 4, 1);
    chan_m1_getData->keepMode = 1;

    ChanBase* chan_jj_relay_loop_k = new ChanBase(16, 0, 1);  // Relay channel in loop k for chan_jj_lc
    chan_jj_relay_loop_k->keepMode = 1;

    // loop j
    ChanBase* chan_m2_addr = new ChanBase(16, 4, 5);

    ChanBase* chan_mul = new ChanBase(16, 4, 5);

    ChanBase* chan_partialSum_addr = new ChanBase(16, 4, 5);
    /*ChanBase* chan_partialSum_getData = new ChanBase(16, 4);*/

    ChanBase* chan_partialSum = new ChanBase(16, 4, 5);
